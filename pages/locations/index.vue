...
const initMap = async () => {
  ...
  try {
    map.value = await createMap({
      ...
      center: {
        lat: Number(mapSettings.value.center_latitude),
        lng: Number(mapSettings.value.center_longitude),
      },
    ...
    const markers: MapMarkerInput[] = (data.value ?? [])
      .filter(
        (l) =>
          Number.isFinite(Number((l as any).latitude)) &&
          Number.isFinite(Number((l as any).longitude))
      )
      .map((l) => ({
        position: {
          lat: Number((l as any).latitude),
          lng: Number((l as any).longitude),
        },
...
useSeoMeta({
  ...
  description: 'Browse our locations',
  ogTitle: 'Browse Locations',
  ogDescription: 'Browse our locations',
  ogUrl: () => canonicalUrl.value, // ✅ uses computed that’s safe
  twitterCard: 'summary_large_image',
})

// ✅ Schema.org without calling composables inside the objects
useSchemaOrg([
  defineWebPage({
    '@type': 'CollectionPage',
    name: 'Browse Locations',
    description: 'Browse our locations',
    url: canonicalUrl, // passing a computed is fine
  }),
  defineBreadcrumb({
    itemListElement: [
      { name: 'Home', item: siteRoot },
      { name: 'Locations', item: canonicalUrl.value },
    ],
  }),
])

// If you also add an ItemList, compute it in a watcher/effect but
// do NOT call any composables inside the passed objects.
watchEffect(() => {
  if (!data.value?.length) return
  const items = data.value.map((loc, i) => ({
    '@type': 'ListItem',
    position: i + 1,
    url: new URL(`/locations/${loc.slug}`, siteRoot).toString(),
    name: `${loc.title} – ${loc.city}, ${loc.state}`,
    image: loc.heroImage?.original_url,
  }))
  useSchemaOrg([{ '@type': 'ItemList', name: 'Locations', itemListElement: items }])
})
  ...
  >
    <div v-if="data && mapSettings">
      ...
      <p class="text-center text-lg md:text-xl mb-8 mt-4">
        Click on one of the mapped locations or scroll down to see the full list
        of locations.
      </p>
      <MaxWidthContentWrapper class="my-8">
        <!-- Relative wrapper to hold map + overlay without affecting layout or scrolling -->
        <div
          class="relative w-full aspect-[1/1] sm:aspect-[2/1] lg:aspect-[3/1] min-h-[320px]"
        >
      ...
      <div class="flex flex-col md:flex-row justify-center gap-10 mx-10 my-20">
        ...
        <div class="flex flex-col max-w-[46ch]">
          ...
          <p class="text-sm text-center">
            You will need to allow the site to access your location to find the
            nearest locations.
          </p>
          <p v-if="locateError" class="text-sm text-red-600 mt-2 text-center">
            {{ locateError }}
          </p>
        </div>
      </div>
          ...
          <CardGallery class="my-8">
            <LocationCard
              ...
              :estimated-driving-time="
                travel[location.slug]?.durationText || ''
              "
