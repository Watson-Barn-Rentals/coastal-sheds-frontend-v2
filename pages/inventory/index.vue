...
import { getDrivingDistances, getUserPosition, type DistanceResult } from '~/services/google-maps'
...
const { data, pending, error, refresh } = await useAsyncData<InventoryItem[]>('inventory-list', getInventoryList)
const { data: mapSettings, pending: mapPending, error: mapError, refresh: mapRefresh } = await useAsyncData<LocationsMapSettings>('locations-map-settings', getLocationsMapSettings)


useSeoMeta({ title: 'Browse Inventory', description: 'Shop in-stock inventory by model, size, location, and more!' })
...
const distanceMetersByLocationSlug = computed<Record<string, number | null>>(() =>
  Object.fromEntries(
    Object.entries(locationDistanceBySlug.value).map(([slug, res]) => [slug, res?.meters ?? null])
  )
)
...
const {
  ...
  clearChip,
  reset,
} = useInventoryFilters(data as Ref<InventoryItem[] | null>, { distanceMetersByLocationSlug })
...
watch(sortMode, async (m) => { if (m === 'distance-from-user') await ensureDistances() })
watch(filtered,  async () => { if (sortMode.value === 'distance-from-user') await ensureDistances() })
...
<template>
  <PageDataGate :sources="[{ data, pending, error, refresh }, { data: mapSettings, pending: mapPending, error: mapError, refresh: mapRefresh }]">
    <Heading text="Browse In-Stock Inventory" heading-level="h1" text-alignment="center" class="mt-12 md:mt-24 mb-8" />
    ...
    <MaxWidthContentWrapper>
      ...
      <CardGallery class="my-8">
        ...
        >
          <p class="text-center text-xl">{{ hiddenByFilters }} {{ hiddenByFilters === 1 ? 'Item' : 'Items' }} Hidden by Filters</p>
          <button class="bg-brand text-white p-2 mx-auto rounded-lg flex gap-1 cursor-pointer transition-all duration-150 hover:scale-105" @click="scrollToFilters">
