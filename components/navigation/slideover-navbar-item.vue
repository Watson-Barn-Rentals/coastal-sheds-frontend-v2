...
const emit = defineEmits<{ (e: 'close-mobile-menu'): void }>()
	...
	>
		...
		>
			<div class="flex gap-1 lg:gap-1.5">
				<UIcon v-if="menuItem.icon" :name="menuItem.icon" class="my-auto h-5 w-5" />
	...
	<div v-else class="flex flex-col gap-2">
		<li class="relative font-title flex gap-2 text-xl cursor-pointer border-b-2 border-transparent pb-1 mr-auto" @click="handleClick">
			<div class="flex gap-1 lg:gap-1.5">
				<UIcon v-if="menuItem.icon" :name="menuItem.icon" class="my-auto h-5 w-5" />
				<span class="select-none text-nowrap my-auto">{{ menuItem.text }}</span>
				<UIcon v-if="menuItem.has_children" name="material-symbols:chevron-right-rounded" class="mt-auto hh-5 w-5 duration-300 ease-in-out" :class="{ '-rotate-90': dropdownState, 'rotate-90': !dropdownState }" />
			</div>
		</li>
		...
		<div v-if="dropdownState" class="flex flex-col gap-4 pl-8 pb-4">
			...
			>
				...
				>
					<div class="flex gap-1 lg:gap-1.5">
						<UIcon v-if="subItem.icon" :name="subItem.icon" class="my-auto h-5 w-5" />
