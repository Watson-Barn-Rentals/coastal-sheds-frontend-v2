...
export default defineNuxtConfig({
	...
	routeRules: {
		'/inventory/': {
			...
			headers: {
				...
				// you can set this header via Netlify Edge Functions to control bucketed caching
	...
	hooks: {
		/**
		 * BUILD-TIME: emit `.nuxt/prerender-routes.generated.ts` for AppLink lookups.
		 ...
		 */
  		'nitro:config': async (nitroConfig) => {
  		  ...
  		  // This is the directory behind `#build/*`
  		  const outDir = nitroConfig.buildDir || path.join(process.cwd(), '.nuxt')
		
  		  try {
  		    await writePrerenderRouteModule({ apiRoot, outDir })
  		  } catch (err) {
  		    console.warn('[prerender] Failed to generate prerender route module:', err)
  		  }
  		},

		/**
		 * Prerender ONLY the CMS-defined list.
		 */
